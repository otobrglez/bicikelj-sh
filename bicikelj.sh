...
distance () {
  lat_1="$1"; lon_1="$2"; lat_2="$3"; lon_2="$4"
  delta_lat=`bc <<<"$lat_2 - $lat_1"`; delta_lon=`bc <<<"$lon_2 - $lon_1"`
  lat_1="`deg2rad $lat_1`"; lon_1="`deg2rad $lon_1`"
  lat_2="`deg2rad $lat_2`"; lon_2="`deg2rad $lon_2`"
  delta_lat="`deg2rad $delta_lat`"; delta_lon="`deg2rad $delta_lon`"
...
get_stations () {
  ...
  curl -G \
    ...
    -XGET -s 'https://prominfo.projekti.si/web/api/MapService/Query/lay_bicikelj/query?geometry=$geometry&outFields=*'
}
...
stations_list | while IFS=',' read LINE; do
  current_latitude=`echo $current_location | cut -d "," -f 1`; current_longitude=`echo $current_location | cut -d "," -f 2`
  latitude=`echo $LINE | cut -d "," -f 1`; longitude=`echo $LINE | cut -d "," -f 2`
